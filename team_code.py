...
#
# ---------------------------------------------------------------------------
# What changed relative to the previous (0.632) entry, and why
# ---------------------------------------------------------------------------
#
#  1. FULL NIGHT, NOT THE FIRST HOUR.  The old entry truncated at 3600 s, which
#     is sleep onset plus roughly one NREM cycle and almost no REM.  The whole
#     recording is now processed (capped at MAX_HOURS as a sanity guard).
#
#  2. EPOCH-LEVEL FEATURES WITH DISTRIBUTIONAL AGGREGATION.  Features are
#     computed per 30 s epoch and then summarised across the night by mean,
#     std, p10 and p90.  The spread statistics are the fragmentation signal;
#     a single whole-night mean destroys them.
#
#  3. STAGE-CONDITIONED FEATURES.  Sigma power in N2 is spindle activity and
#     delta in N3 is slow-wave activity.  Averaged over a mixed night both are
#     diluted.  Per-stage means and explicit N3-vs-REM / NREM-vs-W contrasts
#     are now emitted.
#
#  4. SCALE-INVARIANT FEATURES ONLY.  Absolute std / MAV / RMS / band powers /
#     percentiles track amplifier gain and EDF units, which differ by site.
#     Every feature here is either a ratio, a normalised spectral quantity, or
#     an amplitude expressed in units of the channel's own night-long IQR.
#
#  5. MISSING MEANS NaN, NOT ZERO.  Zero-filling a missing block and then
#     standardising turns "absent" into an extreme outlier, and channel
#     availability is a near-perfect site fingerprint.  Missing blocks are NaN
#     and HistGradientBoostingClassifier learns a default split direction.
#     StandardScaler is gone (trees do not need it).
#
#  6. ALL EEG CHANNELS AVERAGED, not first-available.  Which channel "first"
#     resolves to depends on the site montage, so the old feature meant a
#     different thing per record.
#
#  7. NEW FEATURE BLOCKS: sigma peak frequency and spindle density; arousal
#     temporal structure (inter-arousal intervals, first/second half of night,
#     per stage); real HRV from R-peak detection rather than generic spectral
#     features on the ECG trace.
#
#  8. FAIL-LOUD channel_table.csv GUARD.  Previously a missing table raised
#     inside a bare except, silently zeroing every physiological feature at
#     inference only, which alone produces a chance-level leaderboard from a
#     model that trained fine.  It now raises, and fallback counts are printed.
#
#  9. FOLD ENSEMBLE + HONEST OFFLINE ESTIMATE.  Five stratified fold models are
#     averaged at inference.  Training also prints a site-grouped out-of-fold
#     AUROC with demographics-only / signal-only ablations, so cross-site
#     generalisation can be checked without spending a submission.
#
# The harness (train_model.py / run_model.py / helper_code.py) is UNCHANGED.
# Only this file is edited.  train and inference share one feature-assembly
# code path (`assemble_features`) so the dimension cannot silently drift.
#
################################################################################
#
# Libraries
